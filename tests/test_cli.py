...
def test_modelo_determinado_y_coincide_devuelve_0(tmp_path, capsys):
    """Ej. 7: los dos determinados, con los mismos titulares (el criterio distinto no cuenta)."""
    assert main([escribir(tmp_path, V.leer_ejemplo_modelo())]) == 0
...
def test_un_solo_regimen(tmp_path, capsys):
    """Ej. 1 con un solo régimen: el AMLR queda determinado; España, en el supletorio."""
    ruta = escribir(tmp_path, EJ1)
...
def test_opcion_json_emite_el_informe(tmp_path, capsys):
    ...
    assert main(["--json", escribir(tmp_path, datos)]) == 0
...
def test_opcion_json_con_un_solo_regimen(tmp_path, capsys):
    assert main(["--json", "--regimen", "espana", escribir(tmp_path, EJ3)]) == 0
    emitido = json.loads(capsys.readouterr().out)
