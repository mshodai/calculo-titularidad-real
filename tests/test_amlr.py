...
def test_h3_con_varios_huecos_juntos():
    """C35: ningún hueco solo controla Y, pero todos juntos sí, y Y tiene el 30 % de S."""
    r = calcular(DOS_HUECOS)
    assert set(titulares(r)) == {"R"} and not de(r, "H1") and not de(r, "POS-HUECO")
    (h3,) = de(r, "H3")
    assert h3.ids == ("NO_IDENTIFICADO(Y)", "NO_IDENTIFICADO(Z)") and "cumpliría A3" in h3.mensaje
